...
class PodcastFeedGenerator:
    ...
    def _add_channel_metadata(self, channel: ET.Element):
        ...
        # Atom self-link — теперь указываем на raw GitHub
        filename = f"{self.podcast.brand_id or self.podcast.rubric_id or 'feed'}.xml"
        self.xml.add_ns_element(
            channel, ATOM_NS, "link",
            href=f"https://raw.githubusercontent.com/hrodmig/rss/main/docs/{filename}",
            rel="self",
...
def save_podcast_feed(podcast: PodcastModel, feed_str: str):
    """Сохраняет фид в docs/ для последующего коммита"""
    os.makedirs("docs", exist_ok=True)
    
    # Определяем имя файла
    file_id = podcast.brand_id or podcast.rubric_id or "unknown"
    ...
    logger.info(f"✅ Saved {filename} ({len(feed_str)//1024} KB)")
...
def create_station_feeds(station: StationModel):
    for podcast in station.podcasts:
        ...
        try:
            ...
            save_podcast_feed(podcast, feed_str)
        except Exception as e:
